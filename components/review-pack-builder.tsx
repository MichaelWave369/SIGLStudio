  ...
  return (
    <div className="space-y-4">
      <div className="panel flex flex-wrap items-center gap-2 text-sm">
        ...
        <button className="rounded border border-line px-3 py-1" onClick={() => downloadTextFile(`${selected.title}.review-summary.json`, JSON.stringify({ export_version: "1.0", created_at: new Date().toISOString(), included_item_count: selected.included_items.length, theme_id: selected.theme_id, pack_hash: selected.pack_hash }, null, 2))}>Export Review Summary</button>
        <button className="rounded border border-line px-3 py-1" onClick={() => downloadTextFile(`${selected.title}.handoff.json`, JSON.stringify(createHandoff({ handoff_type: 'review-pack', source_context: 'review-packs', payload: selected as unknown as Record<string, unknown>, notes: selected.notes, theme_id: selected.theme_id, schema_version: '1.0' }), null, 2))}>Export Handoff</button>
      </div>
