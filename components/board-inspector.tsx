...
export function BoardInspector({ block, onUpdate, onDelete, onDuplicate }: { block: BoardBlock | null; onUpdate: (next: BoardBlock) => void; onDelete: () => void; onDuplicate: () => void }) {
  ...
  return (
    <div className="panel space-y-2 text-sm">
      ...
      <div className="grid grid-cols-2 gap-2">
        <input type="number" value={block.w} onChange={(e) => onUpdate({ ...block, w: Number(e.target.value) })} className="rounded border border-line bg-slate-950/50 p-2" />
        <input type="number" value={block.h} onChange={(e) => onUpdate({ ...block, h: Number(e.target.value) })} className="rounded border border-line bg-slate-950/50 p-2" />
      </div>
      <div className="flex items-center gap-2 text-xs">
        <button className="rounded border border-line px-2 py-1" onClick={() => onUpdate({ ...block, locked: !block.locked })}>{block.locked ? "Unlock" : "Lock"}</button>
        <span className="text-muted">Layer: {block.layer_id ?? "default"}</span>
      </div>
