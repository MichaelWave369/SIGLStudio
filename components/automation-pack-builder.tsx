    ...
    <div className="panel space-y-2 text-sm">
      <button className="rounded border border-line px-2 py-1 text-xs" onClick={() => {
        const pack: AutomationPack = {
          ...
          recipes: [{ id: crypto.randomUUID(), name: "Validate + Artifact", description: "", source_scope: "global", output_target: "artifact-set", schema_version: "0.8", actions: [{ id: crypto.randomUUID(), kind: "validate", params: {} }, { id: crypto.randomUUID(), kind: "create-artifact-set", params: { preset: "review-pack" } }] }]
        };
        saveAutomationPacks([pack, ...packs]);
        setRefresh((v) => v + 1);
      }}>Create Automation Pack</button>
      {packs.map((pack) => <p key={pack.id} className="text-xs">{pack.name} ({pack.recipes.length} recipes)</p>)}
    </div>
  );
}
