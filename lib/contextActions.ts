...
export function getContextActions(pathname: string): ContextAction[] {
  ...
  if (pathname.startsWith('/recipes')) return [{ label: 'Open in Analytics', route: '/analytics' }, { label: 'Open in Publishing', route: '/publishing' }];
  if (pathname.startsWith('/publishing')) return [{ label: 'Open in Review Packs', route: '/review-packs' }, { label: 'Open in Present', route: '/present' }];
