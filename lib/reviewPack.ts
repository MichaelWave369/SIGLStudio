...
export function createReviewPack(input: { project_id: string; title: string; notes: string; included_items: string[]; theme_id?: string }): ReviewPack {
  ...
  return { id: crypto.randomUUID(), project_id: input.project_id, title: input.title, notes: input.notes, included_items: input.included_items, sections, created_at: now, updated_at: now, pack_hash, theme_id: input.theme_id ?? "observatory", schema_version: "0.7" };
}
